...
import com.odesa.musicMatters.core.data.playlists.impl.CURRENT_PLAYING_QUEUE_PLAYLIST_ID
...
class PlaylistStoreImplTest {
    ...
    @Test
    fun testFetchFavoriteSongsPlaylist() {
    ...
    @Test
    fun testAddToFavoriteSongsPlaylist() {
...
//        assertTrue( FileAdapter( playlistFile ).read().isNotEmpty() )
    ...
    @Test
    fun testRemoveFromFavoriteSongsPlaylist() {
        ( 0..5 ).forEach { _ ->
            playlistStore.addToFavorites( UUID.randomUUID().toString() )
        }
    ...
    @Test
    fun testFetchRecentlyPlayedSongsPlaylist() {
    ...
    @Test
    fun testFetchMostPlayedSongsPlaylist() {
    ...
    @Test
    fun testAddToMostPlayedSongsPlaylist() {
        testSongs.forEach {
    ...
    @Test
    fun testRemoveFromMostPlayedSongsPlaylist() {
        testSongs.forEach {
            playlistStore.addToMostPlayedSongsPlaylist( it.id )
        }
    ...
    @Test
    fun testSongsInMostPlayedSongsPlaylistAreSortedInDescendingOrderFromMostPlayedToLeastPlayed() {
    ...
    @Test
    fun testSaveCustomPlaylist() {
        testPlaylists.forEach {
            playlistStore.savePlaylist( it )
        }
        assertEquals( 20, playlistStore.fetchEditablePlaylists().size )
    }

    @Test
    fun testDeleteCustomPlaylist() {
        testPlaylists.forEach {
            playlistStore.savePlaylist( it )
        }
    ...
    @Test
    fun testAddSongToCustomPlaylist() {
        testPlaylists.forEach {
            playlistStore.savePlaylist( it )
        }
        testSongs.forEach {
        ...
        val storedCustomPlaylists = playlistStore.fetchEditablePlaylists()
    ...
    @Test
    fun testPlaylistsAreCorrectlyRenamed() {
        testPlaylists.forEach {
            playlistStore.savePlaylist( it )
        }
    ...
    @Test
    fun testPlayingQueueIsSavedCorrectly() {
        assertTrue( playlistStore.fetchCurrentPlayingQueue().songIds.isEmpty() )
        testSongs.forEach {
            playlistStore.addSongIdToCurrentPlayingQueue( it.id )
        }
        assertEquals( testSongs.size, playlistStore.fetchCurrentPlayingQueue().songIds.size )
    }

    @Test
    fun testPlayingQueueIsNotIncludedInOrdinaryPlaylists() {
        assertNull(
            playlistStore.fetchAllPlaylists().find { it.id == CURRENT_PLAYING_QUEUE_PLAYLIST_ID }
        )
    }

    @Test
    fun testCurrentPlayingQueuePlaylistIsClearedCorrectly() {
        testSongs.map { it.id }.forEach { songId ->
            playlistStore.addSongIdToCurrentPlayingQueue( songId )
        }
        playlistStore.clearCurrentPlayingQueuePlaylist()
        assertTrue(
            playlistStore.fetchCurrentPlayingQueue().songIds.isEmpty()
        )
    }
