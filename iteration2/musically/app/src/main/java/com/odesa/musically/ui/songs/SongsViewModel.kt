...
import com.odesa.musically.services.media.extensions.isPlaying
import com.odesa.musically.services.media.extensions.isPrepared
    ...
    private val subscriptionCallback = object : MediaBrowserCompat.SubscriptionCallback() {
        override fun onChildrenLoaded(
            ...
            val itemList = children.map { child ->
                val mediaId = child.description.mediaId!!.toLong()
...
//    }
    ...
    /**
     * This method takes a [Song] and does the following:
     *  - If the song is *not* the currently playing song, then play it directly.
     *  - If the song *is* the currently playing song, check whether *pause* is a permitted command.
     *    If it is, then pause playback, otherwise send "play" to resume playback
     */
    fun playSong( song: Song, pauseAllowed: Boolean = false ) {
        val nowPlayingSong = musicServiceConnection.nowPlayingSong.value
        val transportControls = musicServiceConnection.transportControls
        val isPrepared = musicServiceConnection.playbackState.value.isPrepared
        if ( isPrepared && song.mediaUri == nowPlayingSong.mediaUri ) {
            with ( musicServiceConnection.playbackState.value ) {
                when {
                    isPlaying ->
                        if ( pauseAllowed ) transportControls.pause() else Unit

                    isPlayEnabled -> transportControls.play()
                    else -> {
                        Timber.tag( SongsViewModelTag ).d(
            ...
            transportControls.playFromMediaId( song.mediaUri.toString(), null )
        }
    }
