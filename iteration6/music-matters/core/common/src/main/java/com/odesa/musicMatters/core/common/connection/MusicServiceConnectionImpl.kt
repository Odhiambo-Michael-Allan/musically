...
class TestableMusicServiceConnection(
    ...
    private suspend fun updateMediaCaches() {
        ...
        _cachedGenres.value = retrieveGenresUsing(
            genreMediaItems = connectable.getChildren( MUSIC_MATTERS_GENRES_ROOT ),
            songs = _cachedSongs.value
        )
    ...
    private fun retrieveGenresUsing(
        ...
        genreMediaItems.forEach { genreMediaItem ->
            genreList.add(
                Genre(
                    name = genreMediaItem.mediaMetadata.title.toString(),
                    numberOfTracks = songs.count {
                        it.genre == genreMediaItem.mediaMetadata.title.toString()
                    }
                )
            )
        }
