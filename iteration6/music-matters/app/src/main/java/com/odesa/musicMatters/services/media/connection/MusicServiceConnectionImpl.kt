...
class MusicServiceConnectionImpl( context: Context, serviceComponentName: ComponentName )
    ...
    private suspend fun updateCaches() {
        ...
        _cachedArtists.value = createArtistsUsing( getChildren( MUSIC_MATTERS_ARTISTS_ROOT ).map { it.toArtist() } )
    ...
    private fun createArtistsUsing( artistList: List<Artist> ): List<Artist> {
        val artistsWithTrackCountSet = fillArtistTrackCount( artistList )
        val artistsWithAlbumCountSet = fillArtistAlbumCount( artistsWithTrackCountSet )
        return artistsWithAlbumCountSet
    }

    private fun fillArtistTrackCount( artists: List<Artist> ): List<Artist> {
        val newList = mutableListOf<Artist>()
        artists.forEach { artist ->
            newList.add(
                artist.copy(
                    trackCount = cachedSongs.value.count { it.artists.contains( artist.name ) }
                )
            )
        }
        return newList
    ...
    private fun fillArtistAlbumCount( artists: List<Artist> ): List<Artist> {
        val newList = mutableListOf<Artist>()
        artists.forEach { artist ->
            newList.add(
                artist.copy(
                    albumCount = cachedAlbums.value.count { it.artists.contains( artist.name ) }
                )
            )
        }
        return newList
