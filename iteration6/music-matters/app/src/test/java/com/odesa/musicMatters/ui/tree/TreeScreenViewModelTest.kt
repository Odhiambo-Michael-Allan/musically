...
class TreeScreenViewModelTest {
    ...
    fun testCurrentlyPlayingSongIdIsCorrectlyUpdated() {
        musicServiceConnection.setNowPlaying( trackList.first().mediaItem )
        assertEquals( trackList.first().mediaItem.mediaId, viewModel.uiState.value.currentlyPlayingSongId )
    }
